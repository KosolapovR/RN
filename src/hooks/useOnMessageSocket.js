...
import {
  ...
  DEAL_STATUSES,
  routes,
} from '@cashelec/shared/consts';
import {wallet, transaction} from '@cashelec/shared/schemas';
import {getActionsLog} from '@cashelec/shared/helpers';
import {
  useUserApi,
  useDealApi,
  useDealsApi,
  useWalletsApi,
  useResourcesApi,
} from '@cashelec/shared/hooks/api';
...
export default (location) => {
  ...
  const {cryptoBlockHeight, updateCryptoBlockHeight} = useResourcesApi();
  const {
    walletEntities,
  ...
  const updateBlockHeight = useCallback(
    (data) => {
      const foundIndex = cryptoBlockHeight.findIndex(
        (item) => item.get('currencyAlias') === data.currencyAlias,
      );
  ...
  const updateTransaction = useCallback(
    (data) => {
      ...
      updateTransactionEntities(
        transactionEntities.mergeDeep(fromJS(entities.transaction)),
      );
    },
    [transactionEntities],
  );
        ...
        case WEBSOCKET_EVENT_TYPES.MODEL_CHANGE: {
          switch (data.data.type) {
            case MODEL_CHANGE_TYPES.DEAL: {
              ...
              if (location.pathname === routes.getDealsBase()) {
                ...
                if (newDeals.find((D) => D.get('id') === newDeal.id)) {
                  prevStatus = newDeals
                    .find((D) => D.get('id') === newDeal.id)
                    .get('sellerStatus');
                }
                if (activeDeals.find((D) => D.get('id') === newDeal.id)) {
                  prevStatus = activeDeals
                    .find((D) => D.get('id') === newDeal.id)
                    .get('sellerStatus');
                }
                ...
                if (prevStatus !== newDeal.sellerStatus) {
                  switch (dealStatus) {
                    ...
                    case DEAL_STATUSES.PRE_INITIALIZED: {
                      let newResults = newDeals.size
                        ? newDeals.map((D) => D.get('id'))
                        : List();
                      let newMetaResults = newDealsMeta.size
                        ? newDealsMeta
                        : Map();

                      if (!newResults.includes(newDeal.id)) {
                        newResults = newResults.push(newDeal.id);
                        newMetaResults = newMetaResults.set(
                          'amount',
                          newMetaResults.amount + 1 || 1,
                        );
                      }
                    ...
                    case DEAL_STATUSES.DISPUTE_START: {
                      let activeResults = activeDeals.size
                        ? activeDeals.map((D) => D.get('id'))
                        : List();
                      let activeMetaResults = activeDealsMeta.size
                        ? activeDealsMeta
                        : Map();
                      ...
                      if (prevStatus && newDeals.size) {
                        const newResults = newDeals
                          ...
                          .map((D) => D.get('id'));
                        const newMetaResults = newDealsMeta.set(
                          'amount',
                          newDealsMeta.get('amount', 1) - 1,
                        );
                      ...
                      if (!activeResults.includes(newDeal.id)) {
                        ...
                        activeMetaResults = activeMetaResults.set(
                          'amount',
                          activeMetaResults.amount + 1 || 1,
                        );
                      }
                    ...
                    case DEAL_STATUSES.OUTDATED: {
                      let closedResults = closedDeals.size
                        ? closedDeals.map((D) => D.get('id'))
                        : List();
                      let closedMetaResults = closedDealsMeta.size
                        ? closedDealsMeta
                        : Map();
                      ...
                      if (prevStatus && (activeDeals.size || newDeals.size)) {
                        if (
                          ...
                          const activeResults = activeDeals
                            .filter((D) => D.get('id') !== newDeal.id)
                            .map((D) => D.get('id'));
                          const activeMetaResults = activeDealsMeta.set(
                            'amount',
                            activeDealsMeta.get('amount', 1) - 1,
                          );
                      ...
                      if (!closedResults.includes(newDeal.id)) {
                        ...
                        closedMetaResults = closedMetaResults.set(
                          'amount',
                          closedMetaResults.amount + 1 || 1,
                        );
                      }
            ...
            case MODEL_CHANGE_TYPES.BLOCK_HEIGHT: {
              ...
              if (
                location.pathname ===
                  routes.getSingleDeal({dealId: lastPath}) ||
