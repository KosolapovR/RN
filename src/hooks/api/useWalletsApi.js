...
} from '@cashelec/shared/api/payments';
...
export default () => {
  const selector = useCallback(
    (state) => ({
      ...
      accountPaymentSystems: accountPaymentSystemsSelector(
        state,
        'accountPaymentSystems',
      ),
      rechargeRequest: mapSelector(state, 'rechargeRequest'),

      getPaymentSystemsIsFetching:
      ...
      walletsIsFetching:
        ...
        false,
      postInternalWallets: querySelectors.isPending(state.get('queries'), {
        queryKey: endpoints.getInternalWalletAllUrl(),
      }),
      walletsToSellerIsFetching:
        querySelectors.isPending(state.get('queries'), {
          queryKey: endpoints.getAccountPaymentSystemsUrl(),
        }) ||
        querySelectors.isPending(state.get('queries'), {
      ...
      postBankCardIsFetching:
        querySelectors.isPending(state.get('queries'), {
          queryKey: `post${endpoints.getBankCardUrl()}`,
        }) || false,
      putSendMoneyFromInternalIsFetching:
        querySelectors.isPending(state.get('queries'), {
      ...
      bindActionCreators(
        {
          ...
          putPaymentSystemAccount,
          deletePaymentSystemAccount,
          updateWalletEntities: (walletEntities) =>
            updateEntities({wallet: walletEntities}),
