...
import {get2faCodes} from '@cashelec/shared/api/auth';
import {getActiveTopics, getSubscriptions} from '@cashelec/shared/api/forum';
import {getNews} from '@cashelec/shared/api/news';
import {getWallets, getLastTransactions} from '@cashelec/shared/api/payments';
import {
  getLogHistory,
  getAuthHistory,
  putStep,
...
import {getMarketPairs, getMostPopularPair} from '@cashelec/shared/api/market';
import {getTickers, getEmpoExchangeRate} from '@cashelec/shared/api/offers';
import {getDeals} from '@cashelec/shared/api/deals';
import {getExchangeRateBySymbol} from '@cashelec/shared/api/resources';
import {
  mapSelector,
  useISESelector,
  listSelector,
...
export default () => {
  const selector = useCallback(
    (state) => ({
      ...
      getLogHistoryIsFetching:
        querySelectors.isPending(state.get('queries'), {
          queryKey: endpoints.getLogHistoryUrl({}),
        }) || false,
      getAuthHistoryIsFetching:
        querySelectors.isPending(state.get('queries'), {
