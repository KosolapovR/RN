...
export default () => {
  const selector = useCallback(state => ({
    ...
    isNotificationsUnread: primitiveSelector(state, 'isNotificationsUnread', false),
    ...
    translationFetchingStatus: primitiveSelector(state, 'translationFetchingStatus', ''),
    userLocation: mapSelector(state, 'userLocation'),

    userIsFetching: querySelectors.isPending(
      state.get('queries'),
    ...
    notificationsIsFetching: querySelectors.isPending(
      state.get('queries'),
      { queryKey: endpoints.getNotificationsUrl() },
    ) || false,
    userIsInitializing: !(querySelectors.lastUpdated(
      state.get('queries'),
      { queryKey: `getUser${endpoints.getUserUrl()}` },
    ...
    isUserLoggedIn: !!(querySelectors.lastUpdated(
      state.get('queries'),
    ...
    putAcceptTermsIsFetching: querySelectors.isPending(
      state.get('queries'),
      { queryKey: endpoints.getAcceptTermsUrl() },
    ) || false,
    getTermsIsFetching: querySelectors.isPending(
        state.get('queries'),
        { queryKey: endpoints.getTermsUrl() },
    ) || false,
    isUserMessagesFetching: querySelectors.isPending(
      state.get('queries'),
      { queryKey: endpoints.getForumUserMessages() },
    ) || false,
    userLocationIsFetching: querySelectors.isPending(
      state.get('queries'),
      { queryKey: endpoints.getUserLocationUrl() },
    ) || false,
    onlineUsersIsFetching: querySelectors.isPending(
      state.get('queries'),
      { queryKey: endpoints.getUsersOnlineUrl() },
    ) || false,
    checkPinIsFetching: querySelectors.isPending(
      state.get('queries'),
  ...
  const actions = useMemo(() => bindActionCreators({
    ...
    putStep,
    putFavoriteCrypto,
    updateNotificationEntities: notificationEntities => updateEntities({
      notification: notificationEntities,
    }),
    updateNotificationResults: notificationResults => updateResults({
      notifications: notificationResults,
      isNotificationsUnread: true,
    }),
    setNotificationsRead: () => updateResults({
      isNotificationsUnread: false,
    }),
    setIsTermsAccept: status => updateResults({
      isTermsAccept: status,
    }),
    setTranslationFetchingStatus: status => updateResults({
  ...
  const updateNotifications = useCallback((newNotification) => {
    ...
    resultsArray = Object.keys(resultsArray).sort((previous, next) =>
      new Date(resultsArray[next].date) - new Date(resultsArray[previous].date));

    actions.updateNotificationEntities(data.notificationEntities
      .mergeDeep(fromJS(entities.notification)));
